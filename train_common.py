...
def early_stopping(stats, curr_count_to_patience, global_min_loss):
    ...
    """
    current_val_loss = stats[-1][1]  # Assuming val_loss is the second entry in the stats for each epoch

    if current_val_loss < global_min_loss:
        # If the current validation loss is lower, update global_min_loss and reset the patience counter
