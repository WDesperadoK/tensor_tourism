...
def Rotate(deg=30):
    ...
    def _rotate(img):
        ...
        # Choose a random degree from -deg to deg
        degree = np.random.randint(-deg, deg)
        # Rotate the image using scipy.ndimage.rotate
        rotated_img = rotate(img, degree, reshape=False, mode='constant', cval=0)
        return rotated_img
...
def Grayscale():
    ...
    def _grayscale(img):
        ...
        # Compute the grayscale values as the mean of the RGB channels
        grayscale_img = np.mean(img, axis=2, keepdims=True)
        # Repeat the grayscale values across the color channels and round to nearest integer
        grayscale_img = np.repeat(grayscale_img, 3, axis=2).astype(np.uint8)
